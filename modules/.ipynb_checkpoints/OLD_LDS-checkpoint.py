...
#magic function that sets the plots to display inline and be stored in the notebook
import pandas as pd
import glob
...
# Feb22: A_ii is allowed to be negative
    ...
    for i in range(ND):
      ...
      G[i,i]=0# Feb22: A_ii is allowed to be negative
...
def update_A_refrecting_bdry(A,h):
    res=np.copy(A)
    n=len(A)
    for i in range(n):
        for j in range(n):
...
def forward_initial(x0, mu_pre, V_pre,  Sigma):
    M = len(Sigma)
    I = np.identity(M)
    #x0mat=np.array([x0]).T
    # initial step
    ...
    vec=np.array([x0-normal_mean])
    k = len(x0)
    
    lamlist=LA.eigvals(normal_cov)
    sumloglam=0
    for i in  lamlist:
        sumloglam+=np.log(abs(i))
    lnc_0=(-0.5 *vec @ LA.inv(normal_cov) @ vec.T)[0,0] - 0.5*k*np.log(2*3.1415926535)  -0.5*sumloglam

    
    #print('0: ',np.log(c_0),lnc_0)
    
    return mu0, V0, lnc_0

def forward_later(x_n, mu_old, V_old, A,  Sigma,Gamma):
    ND = len(Sigma)
    I = np.identity(ND)
    
    #x_n_mat=np.array([x_n]).T
    ...
    vec=np.array([x_n-normal_mean])
 
    print('normal_cov ',normal_cov[0,0], ', ',normal_cov[0,1] ,' vec = ',vec )
    lamlist=LA.eigvals(normal_cov)
    sumloglam=0
    for i in  lamlist:
        sumloglam+=np.log(abs(i))
    ...
    k = len(x_n)
    lnc_n=(-0.5 *vec @ LA.inv(normal_cov) @ vec.T)[0,0] - 0.5*k*np.log(2*3.1415926535)  -0.5*sumloglam

    return mu_n, V_n, lnc_n
   
    
    
def calc_log_likelihood(x, mu_star, V_star, A,   counts_deme, Ne):
    ...
    Sigma = np.diag([ mu_star[i]*(1-mu_star[i])/counts_deme[i,0]  for i in range(ND)]) 
        
        #max([(x[i,0]*(1-x[i,0]))/counts_deme[i,0], 1.0/(counts_deme[i,0]**2)])

    
    mu, V, ln_c0=forward_initial(x[:,0], mu_star, V_star, Sigma)
    
    print('ln_c_0=',ln_c0,' mu', mu[0])
    log_likelihood+=ln_c0
    ...
    for t in range(1,len(x[0])):
        ...
        Sigma = np.zeros((ND,ND))
        for i in range(ND):
            Sigma[i,i] = (Amu[i]*(1-Amu[i]))/counts_deme[i,t]
 
        
        Gamma = np.zeros((ND,ND))
        for i in range(ND):
            Gamma[i,i] = Amu[i]*(1-Amu[i])/Ne[i]
   
        
        mu, V, ln_c = forward_later(x[:,t], mu, V, A,  Sigma,Gamma)
       
        print('ln_c_t=',ln_c,'mu', mu[0])
        log_likelihood+=ln_c 
...
def MCMC(B,A_start,counts_deme, Ne_start,tmax_mcmc,dt, burnin=0.25, h=0.005,beta=1):
    ...
    for t_mcmc in range(tmax_mcmc):
        ...
        A_new = update_A_refrecting_bdry(A_old,h)
        ...
        for lin in range(Nlins):
            x=B[:,lin,:].T            
            LogLH_new+=calc_log_likelihood(x, mu_star, V_star, A_new,  counts_deme, Ne_new)

        # acceptance_prob = min(1, np.exp(Delta)). if-else is used for numerical stability
        Delta =beta*(LogLH_new - LogLH_old)
        if Delta>0:
            acceptance_prob=1
        else:
...
def calc_LH_MCMC(B, A, Ne, counts_deme):
    ...
    for lin in range(Nlins):
        print('lin',lin)
        x=B[:,lin,:].T            
        LogLH_all+=calc_log_likelihood(x, mu_star, V_star, A,  counts_deme, Ne)
...
# def Kfilter(x, mu_star, V_star, A,  counts_deme, Ne, freqave):
...
#     Sigma = np.diag([ freqave*(1-freqave)/counts_deme[i,0]  for i in range(ND)])
...
#         Sigma = np.diag([ freqave*(1-freqave)/counts_deme[i,t]  for i in range(ND)])
...
# def EM_Neff(A, Ezz_all, Ezz_all_shift,Ezz_pre_all, freqave):
#     ND = len(A)
#     aux0=Ezz_all_shift
...
#     Ne_new = np.copy([(freqave*(1-freqave)/aux_sum[i,i]) for i in range(ND)])
...
# def Kalman_EM(B, counts_deme, em_step_max,terminate_th):
#     # B = (T, Nlin, ND) 
#     # counts_deme = (ND,T)
...
#             mu_filter, V_filter,P_filter,  lnLH=Kfilter(x, mu_star, V_star, A_old, counts_deme, Ne_old,freqave)    
#             mu_smoother, V_smoother, J = Ksmoother(A_old, mu_filter, V_filter,P_filter)
            
#             Ez, Ezz_pre, Ezz=    Expvals(mu_smoother, V_smoother, J)    
#             Ezz_all+=np.sum(Ezz[:-1],axis=0)
#             Ezz_all_shift +=np.sum(Ezz[1:],axis=0)
#             Ezz_pre_all+=np.sum(Ezz_pre[1:],axis=0)
#             lnLH_all+=lnLH
...
#         A_old = np.copy(A_new)
#         Ne_new=np.copy(EM_Neff(A_old,  Ezz_all,Ezz_all_shift, Ezz_pre_all, freqave))
#         Ne_old = np.copy(Ne_new)
...
#                 print("ERROR: Marginal LH decreases!")
#             if (lnLH_record[-1] - lnLH_record[-2])/lnLH_record[-2] < terminate_th:#Terminate if LH is not improved 
